...
import { PAGE_HEIGHT, PAGE_WIDTH } from "./canvasPage";
...
type ViewportCanvas = Pick<
  StaticCanvas | Canvas,
  "setViewportTransform" | "requestRenderAll"
>;
...
export class ViewportController {
  ...
  fit(): ViewportState {
    ...
    const zoom = Math.min(
      (this.size.width - inset * 2) / PAGE_WIDTH,
      (this.size.height - inset * 2) / PAGE_HEIGHT,
    );
    this.mode = "fit";
  ...
  setZoomPercent(percent: number): ViewportState {
    if (
      ...
      throw new Error(
        `Zoom must be between ${MIN_ZOOM_PERCENT} and ${MAX_ZOOM_PERCENT} percent`,
      );
    }
    this.mode = "custom";
...
export function viewportStateLabel(state: ViewportState): string {
  const mode =
    state.mode === "fit" ? "Fit" : `${Math.round(state.zoomPercent)} percent`;
...
function centeredTransform(size: ViewportSize, zoom: number): TMat2D {
  return [
    ...
    0,
    zoom,
    (size.width - PAGE_WIDTH * zoom) / 2,
    (size.height - PAGE_HEIGHT * zoom) / 2,
  ];
}
...
function clampTransform(transform: TMat2D, size: ViewportSize): TMat2D {
  ...
  return [
    ...
    0,
    zoom,
    clamp(transform[4], visibleEdge - pageWidth, size.width - visibleEdge),
    clamp(transform[5], visibleEdge - pageHeight, size.height - visibleEdge),
  ];
}
