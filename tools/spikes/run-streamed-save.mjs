...
function parseArgs(args) {
  const options = {
    buildId: undefined,
    outputDir: resolve(root, "src-tauri/target/streamed-save-evidence"),
  };
  for (let index = 0; index < args.length; index += 1) {
    switch (args[index]) {
      ...
      case "--help":
      case "-h":
        console.log("run-streamed-save --build-id REVISION [--output-dir DIR]");
        process.exit(0);
        break;
      default:
...
function main() {
  ...
  const binary = resolve(root, "src-tauri/target/release/examples/streamed_save_spike.exe");
  const environment = {
    ...process.env,
    GAMEBOOK_SPIKE_WINDOWS_RELEASE: os.release(),
    GAMEBOOK_SPIKE_CPU_MODEL: os.cpus()[0]?.model ?? "unreported",
    GAMEBOOK_SPIKE_TOTAL_MEMORY_BYTES: String(os.totalmem()),
  };
  const reports = [];
  ...
  const manifest = {
    schema: "gamebook.streamed-save-evidence-manifest.v1",
    issue: 16,
    applicationBuild: {
      name: "gamebook",
      version: "0.5.3",
  ...
  execFileSync(process.execPath, [
    resolve(root, "tools/spikes/verify-streamed-save-report.mjs"),
    "--manifest",
    manifestPath,
  ], { cwd: root, stdio: "inherit" });
