...
[ADR-0001](../decisions/0001-media-placement-rendering.md) accepts the Fabric offscreen surface because it passes the gate twice while retaining one Fabric composition system for placement, annotations, transforms, hit testing, z-order, page switching, and view-only viewport transforms. Layered DOM remains the measured fallback. The accepted decision does not by itself implement production media behavior.
